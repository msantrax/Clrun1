    ...
    def __init__(self, app, config, qlog = None):
        ...
        for toload in loadlist:
            ...
            if not file in self.loadedfiles.keys():
                ccdscantemp = CCDScan( file+".ccdos", None, config)
        ...
        @self.app.callback(  Output('dummy1', 'hidden'),
        ...
        def update_scanfile(list_of_contents, list_of_names):

            if list_of_contents is not None:
                #
                # test= [self.parse_contents(c, n) for c, n in
                #     zip(list_of_contents, list_of_names)

                data = self.parse_contents(list_of_contents, list_of_names)

                print ('Updating Scan file based on upload {}'.format(list_of_names))
                if list_of_names is None: return dash.no_update, dash.no_update

                file = list_of_names.replace(".ccdos", "")

                if not file in self.loadedfiles.keys():
                    # ccdscantemp = CCDScan( file+".ccdos", None, config)
                    ccdscantemp = CCDScan( file, data, config)
                    self.loadedfiles[file] = ccdscantemp
                    entry = {'label': file, 'value': file}
                    self.fileslist.append(entry)
                    ...
                    print ('File {} was loaded'.format(list_of_names))
                    self.update_scan_selection = True
                    self.currentscan[8] = False # Btactions
                else:
                    print ('File {} is already loaded'.format(list_of_names))
